<template>
    <div
      class="fy-vert-scroll"
      ref="wrapper"
      @touchstart="touchStart"
      @touchmove="touchMove"
      @touchend="touchEnd">
        <div class="fy-vert-list" ref="list" :style="{'min-height': listMinHeightStyle}">
            <div class="fy-vert-pulldown" v-if="isFirstPull">
                <div class="fy-vert-pulldown-txt" v-if="!isLoading">{{pulldownTxt}}</div>
                <div class="fy-vert-pulldown-status" v-else>
                    <span class="fy-vert-loading"></span>
                    <span class="fy-vert-loading-txt">加载中...</span>
                </div>
            </div>
            <slot name="list"></slot>
            <div class="fy-vert-loadmore" >
                <template v-if="!isUnMore && isShowLoadMore">
                    <!-- <div class="fy-vert-load-txt" >{{loadMoreTxt}}</div> -->
                    <div class="fy-vert-load-status" v-if="isLoading">
                        <span class="fy-vert-loading"></span>
                        <span class="fy-vert-loading-txt">加载中...</span>
                    </div>
                </template>
                <template v-else-if="isUnMore">
                    <div class="fy-vert-unloadmore-txt">{{unloadMoreTxt}}</div>
                </template>
            </div>
        </div>
    </div>
