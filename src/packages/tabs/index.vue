...
export default {
  ...
  watch: {
    active(nVal) {
      if (nVal !== this.curActive) {
        this.correctActive(nVal)
      }
    },
  ...
  methods: {
    ...
    onTouchEnd() {
      ...
      if (direction === 'horizontal' && this.offsetX >= minSwipeDistance) {
        ...
        // else if (deltaX > 0 && curActive === 0) {
        //   this.setCurActive(this.tabs.length - 1);
        // } else if (deltaX < 0 && curActive === this.tabs.length - 1) {
        //   this.setCurActive(0);
        // }
      }
  ...
  render(h) {
    ...
    const Nav = this.tabs.map((tab, index) => (
      <div
        ...
        class={[
          'fy-tab',
          {
            'fy-tab--active': index === this.curActive,
            'fy-tab--disabled': tab.disabled,
            'fy-tab--complete': !ellipsis
          }
        ]}
      ...
      >
        <span ref="title" refInFor class={{ 'fy-ellipsis': ellipsis }}>
          {tab.title}
        </span>
      </div>
    ));
    ...
    return (
      <div class={['fy-tabs', `fy-tabs--${type}`]}>
        <div
          ref="wrap"
          style={this.wrapStyle}
          class={['fy-tabs__wrap',  scrollable && 'fy-tabs__wrap--scrollable', { 'fy-hairline--top-bottom': type === 'line' }]}
        >
          <div ref="nav" class={['fy-tabs__nav', `fy-tabs__nav--${type}`]} style={this.navStyle}>
            {this.slots('nav-left')}
            {type === 'line' && <div class={['fy-tabs__line']} style={this.lineStyle} />}
            {Nav}
            {this.slots('nav-right')}
          </div>
        </div>
        <div ref="content" class={['fy-tabs__content', animated && 'fy-tabs__content--animated']}>
          {animated ? (
            <div class={['fy-tabs__track']} style={this.trackStyle}>
              {this.slots()}
            </div>
          ) : (
            this.slots()
